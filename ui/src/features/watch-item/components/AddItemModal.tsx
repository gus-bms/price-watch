...
export function AddItemModal({
  ...
  const presetPatterns = useMemo(
    () =>
      PARSER_PRESETS.filter((p) => selectedPresetIds.includes(p.id)).flatMap(
        (p) => p.patterns,
      ),
    [selectedPresetIds],
  );
  ...
  async function handleSubmitStep3(event: FormEvent) {
    ...
    if (!Number.isFinite(parsedPrice) || parsedPrice < 0) {
      setError(
        "유효한 목표 금액을 입력해주세요 (재입고 모드인 경우 0 입력 가능)",
      );
      return;
    }
    ...
    if (finalPatterns.length === 0) {
      setError("적어도 하나의 파싱 정규식이 필요합니다.");
      return;
    }
  ...
  if (llmAnalyzing || (llmSteps.length > 0 && !llmResult)) {
    ...
    if (!isFinished)
      return <LlmAnalysisOverlay steps={llmSteps} onCancel={handleCancelLlm} />;
  }
        ...
        {step === 2 && (
          <form className={styles.form} onSubmit={handleNextStep2}>
            <label className={styles.field}>
              ...
              >
                특정 옵션(예: M, 270 등)의 재고 유무를 파악하려면 해당 텍스트를
                입력하세요. 입력하지 않으면 전체 재고를 추적합니다.
              </p>
              <input
                className={styles.input}
                value={size}
            ...
            <div className={styles.fieldRow} style={{ marginTop: "1.5rem" }}>
              <button
                className={`${styles.submitBtn} ${styles.fieldFlex}`}
                style={{ background: "#666", flex: 0.3 }}
                type="button"
                onClick={() => setStep(1)}
              >
                이전
          ...
          <form className={styles.form} onSubmit={handleSubmitStep3}>
            ...
            {mode === "edit" && !llmResult && (
              ...
              >
                ...
                >
                  품절 파서가 없거나 오래된 상품은 LLM 재분석으로 재고 감지를
                  복구할 수 있습니다.
                </p>
                <button
                  className={styles.submitBtn}
            ...
            {llmResult?.testRun?.isOutOfStock ? (
              ...
              >
                ...
                >
                  현재 해당 옵션({size || "전체"})은 <b>품절(Sold Out)</b>{" "}
                  상태입니다.
                  <br />
                  목표 가격 달성과 관계없이 재입고 시 알림이 전송되는{" "}
                  <b>[재입고 감지 모드]</b>로 동작합니다.
                </p>
              </div>
            ) : (
              <div
              ...
              >
                ...
                >
                  {llmResult?.testRun?.currentPrice
                    ? `현재 발견된 가격: ₩${llmResult.testRun.currentPrice.toLocaleString()}`
                    : "✅ 대상 품목 추적이 준비되었습니다."}
                </p>
                <p
                  style={{
                ...
                >
                  아래에 알림을 받을 <b>목표 금액</b>을 설정해 주세요.
                </p>
              </div>
            )}
            ...
            {!llmResult?.testRun?.isOutOfStock && (
              <div className={styles.fieldRow}>
                ...
                <label
                  className={`${styles.field} ${styles.fieldFlex}`}
            ...
            <details style={{ marginTop: "1rem", fontSize: "0.85rem" }}>
              ...
              <div style={{ marginTop: "0.5rem" }}>
                ...
                <label className={styles.field}>
                  <span className={styles.fieldLabel}>
                    Custom regex patterns (한 줄에 하나씩)
                  </span>
                  <textarea
                    className={styles.textarea}
                    value={customPatternsText}
                    onChange={(e) => {
                      setCustomPatternsText(e.target.value);
                    }}
                    rows={3}
                  />
                </label>
